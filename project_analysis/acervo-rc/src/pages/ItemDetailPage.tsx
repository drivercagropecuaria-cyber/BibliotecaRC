...
export function ItemDetailPage() {
  ...
  const getFileIcon = (tipo?: string) => {
    if (!tipo) return <File className="w-20 h-20 text-neutral-400" />
    if (tipo.startsWith('image')) return <FileImage className="w-20 h-20 text-accent-400" />
    if (tipo.startsWith('video')) return <FileVideo className="w-20 h-20 text-purple-400" />
    return <File className="w-20 h-20 text-neutral-400" />
  }
  ...
  if (!item) return <div className="text-center py-16"><p className="text-neutral-500">Item nao encontrado</p><Link to="/acervo" className="text-primary-600 hover:underline mt-4 inline-block">Voltar ao Acervo</Link></div>
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <button onClick={() => navigate(-1)} className="flex items-center gap-2 text-neutral-600 hover:text-neutral-900 font-medium">
          <ArrowLeft className="w-5 h-5" /> Voltar
        </button>
        <div className="flex gap-3">
          <button onClick={() => { setEditForm(item); setShowEditModal(true) }} className="flex items-center gap-2 px-4 py-2.5 bg-white border-2 border-neutral-200 rounded-xl hover:bg-neutral-50 font-medium transition-all">
            <Edit className="w-4 h-4" /> Edicao Rapida
          </button>
          <button onClick={() => setShowDeleteModal(true)} className="flex items-center gap-2 px-4 py-2.5 bg-red-50 border-2 border-red-200 text-red-600 rounded-xl hover:bg-red-100 font-medium transition-all">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-5">
          ...
          {item.arquivo_url && (
            <div className="flex gap-3">
              <a href={item.arquivo_url} download className="flex items-center gap-2 px-5 py-3 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-xl hover:shadow-green font-semibold transition-all">
                <Download className="w-4 h-4" /> Download
              </a>
              <a href={item.arquivo_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 py-3 bg-white border-2 border-neutral-200 rounded-xl hover:bg-neutral-50 font-medium">
          ...
          {(item.descricao || item.frase_memoria) && (
            <div className="bg-white rounded-2xl p-5 shadow-glass space-y-4">
              {item.frase_memoria && (
                <div className="p-4 bg-gradient-to-r from-primary-50 to-primary-100 rounded-xl border-l-4 border-primary-500">
                  <p className="text-sm text-primary-600 font-medium mb-1">Frase-memoria</p>
                  <p className="text-primary-800 font-semibold italic">"{item.frase_memoria}"</p>
                </div>
              )}
              {item.descricao && (
                <div>
                  <p className="text-sm text-neutral-500 font-medium mb-2">Descricao</p>
                  <p className="text-neutral-700">{item.descricao}</p>
                </div>
              )}
            </div>
          )}
          ...
          {item.observacoes && (
            <div className="bg-white rounded-2xl p-5 shadow-glass">
              <h3 className="font-semibold text-neutral-900 mb-3 flex items-center gap-2">
                <MessageSquare className="w-5 h-5 text-neutral-400" /> Observacoes
              </h3>
              <p className="text-neutral-600">{item.observacoes}</p>
            </div>
          )}

          <div className="bg-white rounded-2xl p-5 shadow-glass">
            <h3 className="font-semibold text-neutral-900 mb-4">Nucleos</h3>
        ...
        <div className="space-y-5">
          <div className="bg-white rounded-2xl p-5 shadow-glass">
            <h2 className="text-xl font-bold text-neutral-900 mb-3">{item.titulo}</h2>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-glass space-y-3">
            <h3 className="font-semibold text-neutral-900 mb-2">Identificacao</h3>
            ...
            <InfoItem label="Capitulo" value={item.capitulo} icon={Bookmark} />
          </div>

          <div className="bg-white rounded-2xl p-5 shadow-glass space-y-3">
            <h3 className="font-semibold text-neutral-900 mb-2">Localizacao</h3>
            <InfoItem label="Area / Fazenda" value={item.area_fazenda} icon={MapPin} />
            <InfoItem label="Ponto" value={item.ponto} />
            <InfoItem label="Evento" value={item.evento} />
            <InfoItem label="Funcao Historica" value={item.funcao_historica} />
          </div>

          <div className="bg-white rounded-2xl p-5 shadow-glass space-y-3">
            <h3 className="font-semibold text-neutral-900 mb-2">Temas</h3>
            <InfoItem label="Tema Principal" value={item.tema_principal} />
          </div>

          {item.arquivo_nome && (
            <div className="bg-white rounded-2xl p-5 shadow-glass">
              <h3 className="font-semibold text-neutral-900 mb-2">Arquivo</h3>
              <p className="text-sm text-neutral-600 truncate">{item.arquivo_nome}</p>
              {item.arquivo_tamanho && <p className="text-xs text-neutral-500 mt-1">{(item.arquivo_tamanho / 1024 / 1024).toFixed(2)} MB</p>}
            </div>
          )}

          <div className="bg-neutral-50 rounded-2xl p-4 text-xs text-neutral-500">
            <p>ID: {item.id}</p>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-xl font-bold text-neutral-900 mb-2">Confirmar Exclusao</h3>
            <p className="text-neutral-600 mb-6">Tem certeza que deseja excluir "{item.titulo}"? Esta acao nao pode ser desfeita.</p>
            <div className="flex gap-3 justify-end">
              <button onClick={() => setShowDeleteModal(false)} className="px-4 py-2.5 border-2 border-neutral-200 rounded-xl hover:bg-neutral-50 font-medium">Cancelar</button>
              <button onClick={handleDelete} disabled={deleteItem.isPending} className="px-4 py-2.5 bg-red-600 text-white rounded-xl hover:bg-red-700 disabled:opacity-50 flex items-center gap-2 font-medium">
      ...
      {showEditModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl p-6 max-w-2xl w-full shadow-2xl my-8">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-neutral-900">Edicao Rapida</h3>
            ...
            <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2">
              <div>
                ...
                <input type="text" value={editForm.titulo || ''} onChange={(e) => setEditForm({ ...editForm, titulo: e.target.value })}
                  className="w-full px-4 py-2.5 border-2 border-neutral-200 rounded-xl focus:ring-2 focus:ring-primary-500" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Responsavel</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Area/Fazenda</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Ponto</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Nucleo Pecuaria</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Tema Principal</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-neutral-700 mb-1">Tipo Projeto</label>
              ...
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-1">Frase-memoria</label>
                <input type="text" value={editForm.frase_memoria || ''} onChange={(e) => setEditForm({ ...editForm, frase_memoria: e.target.value })}
                  className="w-full px-4 py-2.5 border-2 border-neutral-200 rounded-xl focus:ring-2 focus:ring-primary-500" />
              </div>
              <div>
                <label className="block text-sm font-medium text-neutral-700 mb-1">Observacoes</label>
                <textarea value={editForm.observacoes || ''} onChange={(e) => setEditForm({ ...editForm, observacoes: e.target.value })} rows={3}
                  className="w-full px-4 py-2.5 border-2 border-neutral-200 rounded-xl focus:ring-2 focus:ring-primary-500" />
              </div>
            </div>
            <div className="flex gap-3 justify-end mt-6 pt-4 border-t border-neutral-200">
              <button onClick={() => setShowEditModal(false)} className="px-5 py-2.5 border-2 border-neutral-200 rounded-xl hover:bg-neutral-50 font-medium">Cancelar</button>
              <button onClick={handleSaveEdit} disabled={updateItem.isPending} className="px-5 py-2.5 bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-xl hover:shadow-green disabled:opacity-50 flex items-center gap-2 font-semibold">
